...
if (!file.exists("results/dates.rda")){
  dates = read.csv("data/gbie14Csub/dates/archdates.csv",
                 ...
                 strip.white = TRUE)
  head(dates)
  save(dates, file = "results/dates.rda")
}

# Make histogram plot
if (!file.exists("results/hist.pdf")){
  ...
  pdf(file="results/hist.pdf", width=10, height=5)
  ...
  pdf(file="results/hist_presentation.pdf", width=10, height=5)
  par(mar=c(4.5, 4.5, 1, 1) + 0.1)
  wtd.hist(dates$CRA[dates_on_range], breaks=breaks, plot=T, weight = w[dates_on_range],
...
if (!file.exists("results/caldates.rda")){
  ncores = 30
  ...
  caldates = calibrate(x = dates$CRA,
                       ...
                       calMatrix = TRUE)
  stopCluster(cl)
  save(caldates, file = "results/caldates.rda")
}

# Make SPD plot
if (!file.exists("results/spd.rda")){
  bins = binPrep(sites = dates$SiteID, ages = dates$CRA, h = 100)
  allspd = spd(x = caldates, 
               ...
               runm = 100)
  save(bins, allspd, file = "results/spd.rda")
  pdf(file="results/spd.pdf", width=10, height=5)
  par(mar=c(4.5, 4.5, 1, 1) + 0.1)
  ...
  pdf(file="results/spd_presentation.pdf", width=10, height=5)
  par(mar=c(4.5, 4.5, 1, 1) + 0.1)
  plot(allspd$grid$calBP, allspd$grid$PrDens, xlim = time_range_BP,
...
if (!file.exists("results/sumstats.rda")){
  bins = binPrep(sites = dates$SiteID, ages = dates$CRA, h = 100)
  w = apply(as.array(bins), 1, function(x) 1/sum(bins == x))
  ...
  save(all_sumstats_c14, all_sumstats_spd, file="results/sumstats.rda")
}
